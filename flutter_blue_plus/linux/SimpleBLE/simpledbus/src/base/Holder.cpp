...
std::string Holder::_represent_type(Type type, std::experimental::fundamentals_v1::any value) {
    std::ostringstream output;
    output << std::boolalpha;

    switch (type) {
        case BOOLEAN:
            output << std::experimental::any_cast<bool>(value);
            break;
        case BYTE:
            output << std::experimental::any_cast<uint8_t>(value);
            break;
        case INT16:
            output << std::experimental::any_cast<int16_t>(value);
            break;
        case UINT16:
            output << std::experimental::any_cast<uint16_t>(value);
            break;
        case INT32:
            output << std::experimental::any_cast<int32_t>(value);
            break;
        case UINT32:
            output << std::experimental::any_cast<uint32_t>(value);
            break;
        case INT64:
            output << std::experimental::any_cast<int64_t>(value);
            break;
        case UINT64:
            output << std::experimental::any_cast<uint64_t>(value);
            break;
        case DOUBLE:
            output << std::experimental::any_cast<double>(value);
            break;
        case STRING:
        case OBJ_PATH:
        case NONE:
        ...
        case DICT:
        case SIGNATURE:
            output << std::experimental::any_cast<std::string>(value);
            break;
    }
    return output.str();
}
...
std::experimental::fundamentals_v1::any Holder::get_contents() const {
    ...
    switch (_type) {
        ...
        default:
            return std::experimental::fundamentals_v1::any(); 
    }
}
...
void Holder::dict_append(Type key_type, std::experimental::fundamentals_v1::any key, Holder value) {
    if (key.type() == typeid(const char*)) {
        key = std::string(std::experimental::any_cast<const char*>(key));
    }
...
std::map<T, Holder> Holder::_get_dict(Type key_type) const {
    ...
    for (auto& [key_type_internal, key, value] : holder_dict) {
        if (key_type_internal == key_type) {
            output[std::experimental::any_cast<T>(key)] = value;
        }
    }
    return output;
}
